...
You will be provided with:
- User Query: the main question asked by the user.
...
prompt_sql_val = ChatPromptTemplate.from_messages([
    (
        "system",
        """
You are the SQL Validation & Cleanup agent in a Text-to-SQL pipeline.
Your job is to ensure the SQL from the previous node is executable, structurally correct, and follows core safety conventions.

You will be provided with:
- 'User Query': the user's natural language question.
- 'Generated SQL': the SQL query produced by the SQL generation node.

You will output:
- 'validated_sql': a corrected and fully validated SQL query.
- 'validation_reasoning': a short explanation of the fixes
"""
    ),
      ...
      (
        ...
        '''
### Task
Your job is to take the SQL from the previous step and make sure it is **valid, executable, and structurally clean**.
You are **not** rewriting business logic — only validating and fixing SQL structure.
...
- Remove ANY stray symbols: backticks, fences, markdown, text outside SQL.
- No extra quotes around the entire SQL.
- No parentheses wrapping the entire query.
- Ensure it can run directly in a SQL engine.

### 2. Join Corrections
- Default to **LEFT JOIN** unless an INNER JOIN is clearly required.
- Ensure join paths are valid and not missing ON conditions.
- Do NOT add new tables or remove tables.

### 3. Strict Data Preservation
- **Under no circumstances** change filter values in WHERE/HAVING clauses. You do NOT have schema context, so keep all filter values exactly as they appear in the generated SQL.
- If the user question uses different wording for the same filter (e.g., "New York City" vs "NYC")), **do not** replace the filter values, preserve the values from the previous node.
- Do NOT remove, add, rename, or modify columns in SELECT.  
- Do NOT add or remove tables.  
- Do NOT add new metrics or expressions.  
- Only fix broken structure, joins, or formatting.

Correct behavior:
- If generated SQL WHERE state = 'NYC' and user asks for "New York City", keep WHERE state = 'SP' unchanged.
- Reasoning should note: "User uses 'New York City' wording; SQL filters use 'NYC' - preserving SQL literals because validator lacks schema mapping."

Incorrect behavior:
- Rewriting WHERE state = 'NYC' to WHERE state = 'New York City'

### 4. Ordering Logic
- If the **question explicitly or implicitly** refers to a ranking, trend, highest/lowest values, time sequence, or any situation where ordering is naturally expected:
  - Add a sensible `ORDER BY` clause based on **existing SELECT columns only**.
- Examples:
  - "top sellers", "highest sales", "lowest ratings" : order by corresponding metric.
  - "monthly trend", "yearly pattern", "sales over time" : order by month/year/time column.
- If no natural ordering is implied → **do NOT add ORDER BY**.
- Never invent new columns for ordering.

### 5. CTE Handling
- If the SQL has complex nested subqueries → rewrite using CTEs for readability.
- If not complex → no CTEs required.

### 6. Alias & Formatting Rules
- Aliases must NOT be SQL reserved words (e.g., month, year, sum, count).
- Apply clean indentation:
  - SELECT …
  - FROM …
  - JOIN … (indented one level)
  - WHERE …
  - GROUP BY …
  - ORDER BY …
...
## What NOT to do
- Do NOT reinterpret the user question.
- Do NOT change filter values or add new filters.
- Do NOT introduce new columns, tables, metrics, or logic.
- Do NOT wrap anything in ```sql fences.
- Do NOT guess missing schema.
...
{{
  "validated_sql": <final_sql_here>,
  "validation_reasoning": <short_explanation_here>
}}
...
Fixed join types, cleaned formatting, and removed stray characters.
...
def sql_val(state: SQLState) -> SQLState:
     
    chain = prompt_sql_val | llm
    ...
    state["validation_reasoning"] = output["validation_reasoning"]
...
graph.add_node("sql_val", sql_val)
...
graph.add_edge("sql_gen", "sql_val")
graph.add_edge("sql_val", END)
...
def run_text2sql(question: str, debug: bool = False) -> dict:
    ...
    if debug:
        # Return full state for local testing or inspection
        return response
    else:
        # Return only the answer for LangSmith evaluation
        return {"answer": response["validated_sql"]}